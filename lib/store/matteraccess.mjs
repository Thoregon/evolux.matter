...
export default class MatterAccess {
    ...
    isDecorated(prop) {
        return gundecorator.hasOwnProperty(prop);
    }
...
const root = MatterAccess.observe(globalThis.universe ? globalThis.universe.gun : {});
...
const gundecorator = {
        ...
        return (item) => {
            return new Promise((resolve, reject) => {
                try {
                    ...
                    let node = gunnode.set(item);
                    let px = MatterAccess.observe(node, access);
                    resolve(px);
                } catch (e) {
                    reject(e);
                }
    ...
    // Hint: Just modifying the returned object does not modify the DB. Use obj[REF] to change persistent data
    // Hint: Calling val() multiple times to a node always delivers the same ( === ) object
    val(gunnode, proxy, access) {
        return new Promise(resolve => {
            gunnode.once((data, key) => {
                if (exists(data)) {
                    // if (data && typeof data === 'object' && !data[REF]) Object.defineProperty(data, REF, { value: access,  configurable: false, enumerable: false, writable: false })
                    resolve(data);
                } else {
                    resolve(undefined);
                }
